...
COMMANDS = {
    '📕 Основная информация 📕': ['🏢 Информация о компании 🧑‍💻', '🧑‍🎓 Обучение 📖', 'Отмена'],
    '👨‍🏫 Связь и кураторская поддержка 👨‍🏫': ['💬 Информация про дискорд 👨‍🏫', '🤗 Кураторская поддержка 👋', 'Отмена'],
...
def command_processing(message):
    if message.text in COMMANDS:
        ...
        bot.send_message(message.chat.id, 'Выберите вопрос:', reply_markup=create_keyboard(buttons))
...
--------------------
''')
    bot.send_message(message.chat.id, 'Ваш вопрос отправлен Администрации. Ожидайте ответа!')
